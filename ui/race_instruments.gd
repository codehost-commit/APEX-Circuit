...
func _draw() -> void:
	...
	draw_string(font, Vector2(120, 101), "%03d" % ApexStyle.speed_value(car.speed_mps), HORIZONTAL_ALIGNMENT_LEFT, -1, 63, ApexStyle.WHITE)
	draw_string(font, Vector2(265, 101), ApexStyle.speed_suffix(), HORIZONTAL_ALIGNMENT_LEFT, -1, 17, ApexStyle.MUTED)
	...
	draw_string(font, Vector2(22, 182), "STR", HORIZONTAL_ALIGNMENT_LEFT, -1, 16, ApexStyle.MUTED)
	draw_line(Vector2(74, 176), Vector2(size.x - 22, 176), Color("#344650"), 5)
	draw_line(Vector2(steer_x, 172), Vector2(steer_x, 181), ApexStyle.MUTED, 1)
	draw_circle(Vector2(steer_x + car.steering_input * (width - 55) * 0.5, 176), 4, ApexStyle.WHITE)
	if GameState.telemetry_enabled:
		...
		for index in mini(4, wheels.size()):
			...
			var y := 205.0 + floorf(float(index) / 2.0) * 41.0
		...
		draw_string(font, Vector2(22, 297), "DAMAGE %02d%%" % int(car.damage * 100), HORIZONTAL_ALIGNMENT_LEFT, -1, 14, ApexStyle.RED if car.damage > 0.5 else ApexStyle.MUTED)
